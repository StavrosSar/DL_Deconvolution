...
def resize_conserve_flux(img, size):
    ...
    '''
    This function resizes an image while preserving its flux. Flux conservation ensures that the total amount of light in the image stays the same even if the image's size changes.
    Input: The original image img and the target size.
    Output: The resized image scaled to preserve the total flux.
    '''
...
def ir2tf(imp_resp, shape):
    ...
    '''
    This function converts an impulse response (Point Spread Function, PSF) into its transfer function in the Fourier space. 
    This is important because deconvolution is often easier to perform in Fourier space.
    Input: Impulse response imp_resp and the desired output shape.
    Output: The transfer function of the impulse response in Fourier space.
    '''
...
def laplacian_func(shape):
    ...
    '''
    This function generates a Laplacian regularization kernel and returns its transfer function. 
    The Laplacian kernel enhances edges and smoothes out image noise, making it useful for image regularization in deconvolution tasks.
    Output: A Laplacian kernel and its corresponding transfer function in Fourier space.
    '''
...
def wiener(image, psf, balance, laplacian=True):
    ...
    It restores a noisy, blurred image based on the PSF (Point Spread Function) in Fourier space, with an optional Laplacian regularization term. 
    The Wiener filter balances image restoration with noise suppression
...
def deconv_sunet(noisy, 
    ...
    for i in range(noisy.shape[0]): # loop over samples
        for ch in range(noisy.shape[1]): # loop over channel
