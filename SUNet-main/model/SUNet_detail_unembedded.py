...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        H, W = self.input_resolution
        B, L, C = x.shape
        print(f"x shape before reshaping: {x.shape}")  # Add this line
        print(f"Expected shapes -> B: {B}, H: {H}, W: {W}, C: {C}")  # Add this line
        assert L == H * W, "input feature has wrong size"
            
        if L != H * W:
            raise ValueError(f"Expected L ({L}) to be equal to H*W ({H*W})")
            
            
        shortcut = x
        x = self.norm1(x)
        if B * H * W * C != x.numel():
            raise ValueError(f"Cannot reshape tensor to shape ({B}, {H}, {W}, {C}) - mismatch in total elements")
        x = x.view(B, H, W, C)
...
class SUNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=3,
        ...
        if self.final_upsample == "bilinear":  # <=== #
            ...
            # Define a different output layer or leave it as None, based on your requirement
            self.output = nn.Identity()  # A placeholder for cases where you don't want an output layer
    ...
    def forward_features(self, x):
        ...
        print("After patch embedding:", x.shape)  # Debugging line
        if self.ape:
        ...
        for layer in self.layers:
            ...
            print(f"x size before block processing: {x.size()}") #EXAMPLE
            x = layer(x)
