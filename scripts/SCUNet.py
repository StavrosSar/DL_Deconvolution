...
def setup_experiment():
    ...
    parser = argparse.ArgumentParser(description='Train SCUNet denoising model')
    ...
    if args.sigma is not None:
        config['physics']['noise_sigma'] = args.sigma
...
def load_and_preprocess_data(config):
        # Support both SCUNet and DiffUNet style configs
    if 'x_path' in config['data']:
...
# =========================
# Visualization & Metrics (same as DiffUNet)
# =========================
...
def save_metrics(y, x, pred, save_dir):
...
def evaluate_model(model, physics, test_loader, device, save_dir):
    ...
    save_metrics(y, x, pred, save_dir)
...
def main():
    ...
    if mode == 'train':
        ...
        evaluate_model(model, physics, test_loader, device, save_dir)

    elif mode == 'test':
        model_path = os.path.join(save_dir, "scunet_model.pth")
        ...
        # Use trainer.test (like DiffUNet)
        test_subset = torch.utils.data.Subset(test_ds, range(min(5, len(test_ds))))
        fast_loader = DataLoader(test_subset, batch_size=1, shuffle=False)
        test_metrics = trainer.test(fast_loader, metrics=[PSNR(), SSIM()])
        print("Test metrics:", test_metrics)

        evaluate_model(model, physics, fast_loader, device, save_dir)
