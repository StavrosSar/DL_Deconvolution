...
def setup_experiment():
    ...
    save_dir = "../results/DiffUNet_images_results"
    os.makedirs(save_dir, exist_ok=True)
    print(f"Results will be saved to: {save_dir}")
    
    parser = argparse.ArgumentParser(description='Train DiffUNet denoising model')
...
def main():
    ...
    # Model setup
    model = dinv.models.ArtifactRemoval(dinv.models.DiffUNet(pretrained=None).to(device))
    
    # Get sigma value from config
    sigma = config["physics"]["noise_level"]
    physics = Denoising(
    ...
    # Define epochs here before using it
    ...
    # Loss function - using Neighbor2Neighbor which is good for self-supervised denoising
    ...
    trainer = dinv.Trainer(
        ...
        epochs=epochs,  # Use the defined variable here
        scheduler=scheduler,
        losses=loss,
        optimizer=optimizer,
        device=device,
